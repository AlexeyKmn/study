import time
import aiofiles
import asyncio
import aiohttp
from bs4 import BeautifulSoup
import os


async def write_file(session, url, name_img):  # корутина сохранения файла
    async with aiofiles.open(f'images/{name_img}', mode='wb') as f:  # асинхронно открываем
        async with session.get(url) as response:
            # iter_chunked - Iterates over data chunks(куски) with maximum size limit
            async for x in response.content.iter_chunked(1024):
                await f.write(x)  # асинхронный метод write (по кускам)
        print(f'Изображение сохранено {name_img}')


async def main():  # корутина main - точка входа
    url = 'https://parsinger.ru/asyncio/aiofile/1/index.html'
    async with aiohttp.ClientSession() as session:  # открываем сессию
        async with session.get(url) as response:  # получаем содержимое
            soup = BeautifulSoup(await response.text(), 'lxml')  # асинхронно (ожидая ответа сервера) передаём lxml
            img_url = [f'https://parsinger.ru/asyncio/aiofile/1/{x["src"]}' for x in soup.find_all('img')]
            tasks = []
            for link in img_url:  # создаём таски для всех ссылок на картинки
                name_img = link.split('/')[7]  # вытаскиваем имена
                task = asyncio.create_task(write_file(session, link, name_img))  # создаём сами таски
                tasks.append(task)
            await asyncio.gather(*tasks)  # скармливаем методу gather


start = time.perf_counter()
asyncio.set_event_loop_policy(asyncio.WindowsSelectorEventLoopPolicy())
asyncio.run(main())
print(f'Cохранено изображений {len(os.listdir("images/"))} за {round(time.perf_counter() - start, 3)} сек')
